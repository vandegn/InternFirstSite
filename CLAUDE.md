...
**The pipeline board only moves by hand.** Scheduling an interview, requesting availability, and a student declining an invite all leave the candidate's column alone — they create the interview and its notifications, nothing more. The triggers that used to move cards (`trg_sync_stage_on_interview_created`, `trg_sync_stage_on_availability_requested`, `trg_sync_stage_on_interview_closed`) are dropped in `migrations/20260803_manual_pipeline_movement.sql`; their functions survive so recreating the triggers is the whole revert. Interview state shows on the card as a chip; where the card *sits* is the employer's statement, not a side effect.

**Offers — the two-step gate on the Offered column.** Dropping a candidate into a stage whose `stage_type` is `offered` confirms twice: the usual move dialog (its button reads "Continue"), then `ExtendOfferModal`, which is what actually sends it. The second step writes an `offers` row, optionally uploads a PDF offer letter, and notifies the student — the generic "Application update" bell entry is suppressed for this move (`updateApplicationStage(..., { notify: false })`) so the offer notification stands alone. The student sees an offer panel on `/dashboard/student/applications` with the letter and Accept/Decline (declining asks again); their answer notifies the employer, deep-linked back to the candidate's card. `offers.status` runs `extended → accepted | declined`, with `withdrawn` as the employer's off-ramp; a partial unique index allows one live offer per application, so a candidate dragged out of Offered and back is just moved, not re-offered. Letters live in `applicant-docs/offer-letters/<applicationId>/`, read through `/api/files/offer/[id]`. The stage dropdowns on `/applications`, `/posted-jobs` and `/pipeline/all` disable offered columns ("send from Pipeline") so the gate can't be walked around.
