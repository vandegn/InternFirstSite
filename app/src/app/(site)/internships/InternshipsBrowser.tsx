...
export default function InternshipsBrowser({
  ...
  // Seeded from the server render, so the first paint (and the HTML Googlebot
  // reads) already contains every card and its /internships/[id] link.
  const [listings, setListings] = useState<Listing[]>(initialListings);
  // False whenever the server handed us rows — those are already painted.
  ...
  // The server already fetched exactly what this effect would ask for on
  // mount. Refetching would blank the grid and throw away the SSR'd HTML.
  // Only skip when the server actually delivered rows — if its query failed we
  // still want the browser to try, so a bad server fetch degrades to the old
  // client-side behaviour instead of an empty board.
