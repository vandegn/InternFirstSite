...
// Verifies the token_hash from the email templates'
// `/auth/confirm?token_hash={{ .TokenHash }}&type=` links (signup confirmation
// and password recovery). POST-only and fired by the interstitial page's
// button, never on page load, so mail scanners that prefetch GETs can't burn
// the single-use token.
const ALLOWED_TYPES = ['email', 'signup', 'recovery'] as const;
...
export async function POST(request: Request) {
  ...
  // Recovery just needs the session cookies set — the reset page picks the
  // session up via getSession() and shows the new-password form.
  if (type === 'recovery') {
    return NextResponse.json({ redirectTo: '/reset-password' });
  }
