...
// Serves private applicant documents (resumes, application file-answers) from
// the private `applicant-docs` bucket. Authorization reuses the tables' RLS:
// the row is selected with the caller's own session, so if RLS hides it the
// caller isn't allowed to see the file either. Only after that gate passes do
// we sign the object with the service role and redirect.
