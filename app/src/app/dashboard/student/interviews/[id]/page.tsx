...
export default function StudentMeetingRoom() {
  ...
  function renderMessage(icon: string, heading: string, body: string) {
    return (
      <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', height: '60vh', gap: 16, textAlign: 'center', padding: 32 }}>
        ...
        <Link href="/dashboard/student" style={{ padding: '8px 20px', borderRadius: 8, background: 'var(--primary)', color: '#fff', fontWeight: 600, fontSize: '0.85rem', textDecoration: 'none' }}>
          Back to dashboard
        </Link>
      </div>
    );
  ...
  if (status === 'loading') return renderMessage('⏳', 'Setting up your interview…', 'This will only take a moment.');
  ...
  if (status === 'not_configured') return renderMessage('🎥', 'Video not available yet', 'Zoom integration is being set up. Your interview is confirmed — check back closer to the scheduled time.');
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: 'calc(100vh - 64px)' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '12px 24px', borderBottom: '1px solid var(--border)', background: '#fff', flexShrink: 0 }}>
        <Link href="/dashboard/student" style={{ display: 'flex', alignItems: 'center', color: 'var(--text-secondary)', textDecoration: 'none', fontSize: '0.82rem', gap: 4 }}>
          ← Dashboard
        </Link>
        <div style={{ width: 1, height: 16, background: 'var(--border)' }} />
        <span style={{ fontSize: '0.88rem', fontWeight: 600 }}>
          {companyName ?? 'Interview'}{listingTitle ? ` — ${listingTitle}` : ''}
        </span>
      </div>
