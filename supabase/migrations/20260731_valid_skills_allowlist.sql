-- 20260731_valid_skills_allowlist.sql
-- Lock student skills to a fixed catalog. After this migration, student_skills.name
-- must exist in valid_skills; free-text/custom skills are no longer possible, so the
-- is_custom column is dropped. Seed matches app/public/skills.json
-- (regenerate with app/scripts/generate-skills-seed.mjs).

begin;

create table if not exists valid_skills (
  name text primary key
);

alter table valid_skills enable row level security;

-- Signed-in users may read the catalog; only service_role can modify it
-- (so students cannot whitelist their own skill before inserting it).
drop policy if exists "Anyone can view valid skills" on valid_skills;
...
-- Drop existing off-catalog skills (old custom entries) so the FK can be added.
-- Pre-launch cleanup: this deletes student_skills rows whose name is not in the catalog.
delete from student_skills
  where name not in (select name from valid_skills);

alter table student_skills drop constraint if exists student_skills_name_fkey;
alter table student_skills
  add constraint student_skills_name_fkey
  foreign key (name) references valid_skills(name) on update cascade;

alter table student_skills drop column if exists is_custom;
